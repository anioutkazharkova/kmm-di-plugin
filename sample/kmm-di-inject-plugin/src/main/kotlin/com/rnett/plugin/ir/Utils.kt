...
public fun IrSymbolOwner.createIrBuilderAt(context: IrPluginContext): DeclarationIrBuilder =
    DeclarationIrBuilder(context, symbol, startOffset, endOffset)

public fun <T> IrSymbolOwner.withBuilder(context: IrPluginContext,
    block: DeclarationIrBuilder.() -> T
): T = createIrBuilderAt(context).run(block)
...
@OptIn(ObsoleteDescriptorBasedAPI::class, kotlin.contracts.ExperimentalContracts::class)
